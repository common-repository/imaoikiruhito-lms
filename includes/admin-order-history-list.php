...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'edit' === $_GET['action'] ) {
		$this->order_history_detail_page();
		$disp = 'detail';
	}
}
if ( 'list' === $disp ) {
...
<div id="iihlms-wrap" class="wrap">
	<?php
	echo '<h1 class="wp-heading-inline">' . esc_html__( '注文履歴', 'imaoikiruhitolms' ) . '</h1>';
	...
	if ( ! isset( $_GET['iihlmspageoffset'] ) ) {
		...
		if ( ! isset( $_GET['iihlms_nonce'] ) ) {
			...
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['iihlms_nonce'] ) ), 'iihlms_nonce_orderhistorylistpage_action' ) ) {
				echo esc_html__( 'アクセスに異常を検出しました。', 'imaoikiruhitolms' );
				exit;
			}
	...
	$max_row_count = $wpdb->get_var(
		$wpdb->prepare(
			'
			SELECT count(order_cart_id) count
			FROM %1s
			WHERE order_status IN ( %s, %s, %s, %s, %s )
			',
			$order_cart_table,
	...
	if ( 1 === $iihlmspageoffset ) {
		...
		echo '<a class="prev-page button" href="' . esc_url( add_query_arg( 'iihlmspageoffset', absint( $prev_page ), wp_nonce_url( admin_url( 'admin.php?page=iihlms_order_history_list_page' ), 'iihlms_nonce_orderhistorylistpage_action', 'iihlms_nonce' ) ) ) . '"><span class="screen-reader-text">' . esc_html__( '前のページ', 'imaoikiruhitolms' ) . '</span><span aria-hidden="true">‹</span></a>';
	}
	...
	if ( $total_page < $iihlmspageoffset + 1 ) {
		...
		echo '<a class="next-page button" href="' . esc_url( add_query_arg( 'iihlmspageoffset', absint( $next_page ), wp_nonce_url( admin_url( 'admin.php?page=iihlms_order_history_list_page' ), 'iihlms_nonce_orderhistorylistpage_action', 'iihlms_nonce' ) ) ) . '"><span class="screen-reader-text">' . esc_html__( '次のページ', 'imaoikiruhitolms' ) . '</span><span aria-hidden="true">›</span></a>';
	...
	echo '<thead>';
	echo '<tr>';
	echo '<th class="manage-column"><span>' . esc_html__( 'ID', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( '登録日時', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( '支払方法', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( '注文番号', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( '金額(税込)', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( '講座名', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( 'ユーザーID', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( 'ユーザー名', 'imaoikiruhitolms' ) . '</span></th><th class="manage-column"><span>' . esc_html__( 'ステータス', 'imaoikiruhitolms' ) . '</span></th>';
	echo '</tr>';
	echo '</thead>';
	echo '<tbody>';
	...
	foreach ( $results as $result_data ) {
		...
		$registered_datetime  = $result_data->registered_datetime;
		...
		$query_array = array(
			'action' => 'edit',
			'iihlms-orderid' => esc_attr( $order_cart_id ),
		);
		echo '<a href="' . esc_url( add_query_arg( $query_array, wp_nonce_url( admin_url( 'admin.php?page=iihlms_order_history_list_page' ), 'iihlms_nonce_orderlistpage_action', 'iihlms_nonce' ) ) ) . '">';
		echo esc_html( $order_cart_id );
		echo '</a>';
		echo '</td>';
		echo '<td>';
		echo esc_html( $registered_datetime );
		echo '</td>';
		echo '<td>';
